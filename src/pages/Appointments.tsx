...
//   onViewDetails?: (appointment: Appointment) => void;
//   onAccept?: (appointmentId: string, doctorId: string) => void;
//   onReschedule?: (appointmentId: string, newDate: string, newTimeSlot: string) => void;
//   showActions?: boolean;
//   isAlreadyAccepted?: boolean;
// }) => {
//   const { user, logout } = useAuth();
...
//   return (
//     <Card
//       className={`mb-2 sm:mb-4 shadow-card-medical cursor-pointer hover:shadow-medical transition-all duration-300 ${appointment.appointment_type === 'emergency' ? 'border-destructive border-2' : ''}`}
//       onClick={() => onViewDetails?.(appointment)}
//     >
//       <CardContent className="p-2 sm:p-4">
//         <div className="flex flex-col sm:flex-row items-start justify-between mb-2 sm:mb-3">
//           <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3 w-full sm:w-auto">
//             <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-gradient-primary flex items-center justify-center text-white font-semibold text-xs sm:text-sm">
//               {appointment.name
//                 ?.split(' ')
//                 .map((n) => n[0])
//                 .join('')
...
//               <h3 className="font-semibold text-foreground text-sm sm:text-base">{appointment.name}</h3>
//               <p className="text-xs sm:text-sm text-muted-foreground">
//                 {appointment.appointment_type}
//               </p>
//               <p className="text-xs sm:text-sm text-muted-foreground">
//                 <span className="font-medium">Appointment ID:</span> {appointment.appointment_id}
//               </p>
//               <p className="text-xs sm:text-sm text-muted-foreground">
//                 <span className="font-medium">Patient ID:</span> {appointment.patient_id || 'N/A'}
//               </p>
//             </div>
//           </div>
//           <Badge
...
//         {appointment.rejected_reason && (
//           <div className="mt-2 sm:mt-3 p-1 sm:p-2 bg-destructive/10 rounded-md">
//             <p className="text-xs sm:text-sm text-destructive">Reason: {appointment.rejected_reason}</p>
//           </div>
//         )}

//         {showActions && appointment.status !== 'rejected' && (
//           <div className="mt-2 sm:mt-4 flex flex-col sm:flex-row gap-2">
//             {appointment.appointment_type !== 'emergency' && !isAlreadyAccepted && (
//               <Button
//                 size="sm"
...
//   const fetchEmergencyAppointments = async (specialization: string) => {
//     try {
//       let url = 'https://api.onestepmedi.com:8000/emergency/payment_conf_appointments/';
...
//       let url = `https://api.onestepmedi.com:8000/appointments/doctor-appointments/?doctor_id=${doctorId}`;
//       if (filterType === 'custom' && customStartDate && customEndDate) {
//         url += `&start_date=${customStartDate}&end_date=${customEndDate}`;
//       } else if (filterType !== 'custom') {
//         url += `&filter_type=${filterType}`;
...
//       toast({
//         title: "Error",
//         description: "Please provide a new date and time slot.",
//         variant: "destructive",
//       });
//       return;
//     }
...
//     try {
//       const response = await fetch(
//         `https://api.onestepmedi.com:8000/appointments/reschedule/${rescheduleAppointmentId}`,
//         {
//           method: 'PUT',
//           headers: {
//             'Content-Type': 'application/json',
...
//         const errorData = await response.json().catch(() => ({}));
//         throw new Error(
//           response.status === 401 ? 'Session expired' : `Rescheduling failed: ${errorData.message || response.statusText}`
//         );
//       }
//     } catch (err: any) {
//       toast({
...
//     if (filterType === 'day') {
//       matchesFilter = aptDate.getTime() === today.getTime();
...
//       matchesFilter = aptDate >= weekStart && aptDate <= weekEnd;
//     } else if (filterType === 'month') {
//       matchesFilter = aptDate.getMonth() === today.getMonth() && aptDate.getFullYear() === today.getFullYear();
//     } else if (filterType === 'custom' && customStartDate && customEndDate) {
//       const startDate = new Date(customStartDate);
//       const endDate = new Date(customEndDate);
//       startDate.setHours(0, 0, 0, 0);
//       endDate.setHours(23, 59, 59, 999);
//       matchesFilter = aptDate >= startDate && aptDate <= endDate;
//       if (!matchesFilter) {
//         console.log(`Appointment ${apt.appointment_id} excluded: ${apt.preferred_date} not in range ${customStartDate} to ${customEndDate}`);
//       }
//     } else if (filterType === 'custom' && (!customStartDate || !customEndDate)) {
//       // If custom filter is selected but dates are not set, show no appointments
//       matchesFilter = false;
//     }
...
//       }
//     ),
//     all: filteredAppointments.filter(
//       (apt) =>
//         apt.status !== 'rejected' &&
//         apt.status !== 'rescheduled' &&
//         apt.appointment_type !== 'emergency'
...
//   return (
//     <div className="space-y-4 sm:space-y-6 p-2 sm:p-4 md:p-6 max-w-[100vw] mx-auto w-full overflow-x-hidden">
//       <div className="text-center sm:text-left">
//         <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-foreground">Appointments</h1>
//         <p className="text-xs sm:text-sm text-muted-foreground">Manage all your appointments and schedule new ones</p>
//       </div>
...
//           value={searchTerm}
//           onChange={(e) => setSearchTerm(e.target.value)}
//           className="pl-10 pr-4 py-2 sm:py-3 border-2 border-primary rounded-lg focus:ring-2 focus:ring-primary w-full"
//         />
//       </div>

//       <Card className="shadow-card-medical max-w-2xl mx-auto">
//         <CardHeader>
//           <CardTitle className="text-sm sm:text-base md:text-lg">Filter Appointments</CardTitle>
//         </CardHeader>
...
//                 className="flex-1 p-2 sm:p-3 border rounded-md text-xs sm:text-sm"
//               />
//               <input
//                 type="date"
//                 value={customEndDate}
...
//             </div>
//           )}
//           <Button onClick={fetchAppointments} className="w-full sm:w-auto bg-primary text-primary-foreground hover:bg-primary/90 text-xs sm:text-sm py-2 sm:py-2">
//             Apply Filter
//           </Button>
//         </CardContent>
//       </Card>
...
//       {isLoading ? (
//         <p className="text-muted-foreground text-center text-sm sm:text-base">Loading appointments...</p>
//       ) : error ? (
//         <p className="text-red-500 text-center text-sm sm:text-base">{error}</p>
//       ) : (
//         <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4 sm:space-y-6">
//           <TabsList className="grid w-full grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-5 bg-card shadow-card-medical gap-1 sm:gap-2 overflow-x-auto justify-items-center scrollbar-hide">
//             <TabsTrigger value="emergency" className="data-[state=active]:bg-primary data-[state=active]:text-primary-foreground text-xs sm:text-sm px-2 sm:px-3 py-2 sm:py-3 min-w-[80px] sm:min-w-[100px]">
//               <span className="truncate">Emergency ({categorized.emergency.length})</span>
//             </TabsTrigger>
//             <TabsTrigger value="upcoming" className="data-[state=active]:bg-primary data-[state=active]:text-primary-foreground text-xs sm:text-sm px-2 sm:px-3 py-2 sm:py-3 min-w-[80px] sm:min-w-[100px]">
//               <span className="truncate">Upcoming ({categorized.upcoming.length})</span>
//             </TabsTrigger>
//             <TabsTrigger value="all" className="data-[state=active]:bg-primary data-[state=active]:text-primary-foreground text-xs sm:text-sm px-2 sm:px-3 py-2 sm:py-3 min-w-[80px] sm:min-w-[100px]">
//               <span className="truncate">All ({categorized.all.length})</span>
//             </TabsTrigger>
//             <TabsTrigger value="rescheduled" className="data-[state=active]:bg-primary data-[state=active]:text-primary-foreground text-xs sm:text-sm px-2 sm:px-3 py-2 sm:py-3 min-w-[80px] sm:min-w-[100px]">
//               <span className="truncate">Rescheduled ({categorized.rescheduled.length})</span>
//             </TabsTrigger>
//             <TabsTrigger value="rejected" className="data-[state=active]:bg-primary data-[state=active]:text-primary-foreground text-xs sm:text-sm px-2 sm:px-3 py-2 sm:py-3 min-w-[80px] sm:min-w-[100px]">
//               <span className="truncate">Rejected ({categorized.rejected.length})</span>
//             </TabsTrigger>
//           </TabsList>

//           {Object.entries(categorized).map(([key, appts]) => (
//             <TabsContent key={key} value={key} className="space-y-2 sm:space-y-4">
//               <Card className="shadow-card-medical">
//                 <CardHeader>
//                   <CardTitle className={
//                     key === 'rejected' ? 'text-destructive' :
//                     key === 'emergency' ? 'text-destructive' :
//                     key === 'rescheduled' ? 'text-info' : 'text-warning'
//                   } style={{ fontSize: '0.875rem', sm: '1rem', md: '1.25rem' }}>
//                     {key === 'all' ? 'All Appointments' : `${key.charAt(0).toUpperCase() + key.slice(1)} Appointments`}
//                   </CardTitle>
//                   <CardDescription className="text-xs sm:text-sm">
//                     Filtered by {key === 'all' ? filterType : key}
//                   </CardDescription>
//                 </CardHeader>
//                 <CardContent className="p-2 sm:p-4">
//                   {appts.length === 0 ? (
//                     <p className="text-muted-foreground text-center text-xs sm:text-sm">No appointments found.</p>
//                   ) : (
//                     appts.map((appointment) => (
//                       <AppointmentCard
//                         key={appointment.appointment_id}
//                         appointment={appointment}
//                         showActions={key === 'upcoming' || key === 'rescheduled' || key === 'emergency' || key === 'all'}
//                         onReject={handleReject}
//                         onViewDetails={handleViewDetails}
//                         onAccept={handleAccept}
...
//       <Dialog open={selectedAppointment !== null} onOpenChange={(open) => !open && setSelectedAppointment(null)}>
//         <DialogContent className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 max-w-xs sm:max-w-md md:max-w-3xl max-h-[80vh] overflow-y-auto p-2 sm:p-4">
//           <DialogHeader>
//             <DialogTitle className="text-lg sm:text-xl">Patient Case Study Form</DialogTitle>
//           </DialogHeader>
//           {selectedAppointment && (
//             <div className="space-y-3 sm:space-y-4 p-2 sm:p-4">
//               <div>
//                 <Label className="block text-xs sm:text-sm font-medium text-muted-foreground mb-1">Patient Name</Label>
//                 <Input value={selectedAppointment.name} readOnly className="text-xs sm:text-sm" />
//               </div>
//               <div>
//                 <Label className="block text-xs sm:text-sm font-medium text-muted-foreground mb-1">Phone Number</Label>
//                 <Input value={selectedAppointment.phone_number} readOnly className="text-xs sm:text-sm" />
//               </div>
//               <div>
//                 <Label className="block text-xs sm:text-sm font-medium text-muted-foreground mb-1">Appointment Date</Label>
//                 <Input value={selectedAppointment.preferred_date} readOnly className="text-xs sm:text-sm" />
//               </div>
//               <div>
//                 <Label className="block text-xs sm:text-sm font-medium text-muted-foreground mb-1">Time Slot</Label>
//                 <Input
//                   value={new Date(`2000-01-01T${selectedAppointment.time_slot}`).toLocaleTimeString([], {
//                     hour: '2-digit',
//                     minute: '2-digit',
//                   })}
//                   readOnly className="text-xs sm:text-sm"
//                 />
//               </div>
//               <div>
//                 <Label className="block text-xs sm:text-sm font-medium text-muted-foreground mb-1">Symptoms</Label>
//                 <Textarea
//                   value={prescription.case_study.symptoms}
//                   onChange={(e) => setPrescription({
//                     ...prescription,
...
//               </div>
//               <div>
//                 <Label className="block text-xs sm:text-sm font-medium text-muted-foreground mb-1">Diagnosis</Label>
//                 <Textarea
//                   value={prescription.case_study.diagnosis}
//                   onChange={(e) => setPrescription({
//                     ...prescription,
...
//               </div>
//               <div>
//                 <Label className="block text-xs sm:text-sm font-medium text-muted-foreground mb-1">Notes</Label>
//                 <Textarea
//                   value={prescription.case_study.notes}
//                   onChange={(e) => setPrescription({
//                     ...prescription,
...
//               </div>
//               <div>
//                 <Label className="block text-xs sm:text-sm font-medium text-muted-foreground mb-1">Diagnostic Tests</Label>
//                 {prescription.tests.map((test, index) => (
//                   <div key={index} className="flex flex-col sm:flex-row items-center mb-2 sm:mb-3 gap-2">
//                     <div className="flex-1">
//                       <Label className="block text-xs sm:text-sm font-medium text-muted-foreground mb-1">Test Name</Label>
//                       <Input
//                         value={test.test_name}
//                         onChange={(e) => updateTest(index, 'test_name', e.target.value)}
//                         placeholder="e.g., Blood Test"
...
//                     </div>
//                     <div className="flex-1">
//                       <Label className="block text-xs sm:text-sm font-medium text-muted-foreground mb-1">Test Description (Optional)</Label>
//                       <Input
//                         value={test.test_description || ''}
//                         onChange={(e) => updateTest(index, 'test_description', e.target.value)}
//                         placeholder="e.g., Complete blood count analysis"
...
//                   </div>
//                 ))}
//                 <Button onClick={addTest} className="mt-2 text-xs sm:text-sm">Add Test</Button>
//               </div>
//               <div>
//                 <Label className="block text-xs sm:text-sm font-medium text-muted-foreground mb-1">Prescriptions</Label>
//                 {prescription.items.map((item, index) => (
//                   <div key={index} className="border p-2 sm:p-4 mb-2 sm:mb-4 rounded-md relative">
//                     <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4">
//                       <div>
//                         <Label className="block text-xs sm:text-sm font-medium text-muted-foreground mb-1">Medicine Name</Label>
//                         <Input
//                           value={item.medicine_name}
//                           onChange={(e) => updatePrescriptionItem(index, 'medicine_name', e.target.value)}
//                           placeholder="Enter medicine name"
...
//                       </div>
//                       <div>
//                         <Label className="block text-xs sm:text-sm font-medium text-muted-foreground mb-1">Dosage Time</Label>
//                         <Input
//                           value={item.dosage_time}
//                           onChange={(e) => updatePrescriptionItem(index, 'dosage_time', e.target.value)}
//                           placeholder="e.g., Morning, Evening"
...
//                       </div>
//                       <div>
//                         <Label className="block text-xs sm:text-sm font-medium text-muted-foreground mb-1">Duration (Days)</Label>
//                         <Input
//                           type="number"
//                           value={item.duration_days}
//                           onChange={(e) => updatePrescriptionItem(index, 'duration_days', parseInt(e.target.value) || 0)}
...
//                       </div>
//                       <div>
//                         <Label className="block text-xs sm:text-sm font-medium text-muted-foreground mb-1">Quantity</Label>
//                         <Input
//                           type="number"
//                           value={item.quantity}
//                           onChange={(e) => updatePrescriptionItem(index, 'quantity', parseInt(e.target.value) || 0)}
...
//                       </div>
//                       <div>
//                         <Label className="block text-xs sm:text-sm font-medium text-muted-foreground mb-1">Cost per Unit</Label>
//                         <Input
//                           type="number"
//                           value={item.cost_per_unit}
//                           onChange={(e) => updatePrescriptionItem(index, 'cost_per_unit', parseFloat(e.target.value) || 0)}
...
//                   </div>
//                 ))}
//                 <Button onClick={addPrescriptionItem} className="mt-2 text-xs sm:text-sm">Add Prescription</Button>
//               </div>
//               <Button
//                 className="w-full bg-success text-success-foreground hover:bg-success/90 text-xs sm:text-sm py-2 sm:py-3"
//                 onClick={handlePrescriptionSubmit}
//               >
//                 Submit Prescription
//               </Button>
...
//           )}
//           <DialogFooter>
//             <Button variant="outline" className="text-xs sm:text-sm" onClick={() => setSelectedAppointment(null)}>
//               Cancel
//             </Button>
//           </DialogFooter>
//         </DialogContent>
...
//       <Dialog open={rejectDialogOpen} onOpenChange={setRejectDialogOpen}>
//         <DialogContent className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 max-w-xs sm:max-w-md p-2 sm:p-4">
//           <DialogHeader>
//             <DialogTitle className="text-lg sm:text-xl">Reject Appointment</DialogTitle>
//           </DialogHeader>
//           <div className="space-y-3">
//             <div>
//               <Label className="block text-xs sm:text-sm font-medium text-muted-foreground mb-1">Appointment ID</Label>
//               <Input value={rejectAppointmentId || ''} readOnly className="text-xs sm:text-sm" />
//             </div>
//             <div>
//               <Label className="block text-xs sm:text-sm font-medium text-muted-foreground mb-1">Reason for Rejection</Label>
//               <Textarea
//                 value={rejectReason}
//                 onChange={(e) => setRejectReason(e.target.value)}
//                 placeholder="Enter reason for rejection..."
...
//           </div>
//           <DialogFooter>
//             <Button variant="outline" className="text-xs sm:text-sm" onClick={() => {
//               setRejectDialogOpen(false);
//               setRejectReason('');
//               setRejectAppointmentId(null);
//             }}>
...
//         <DialogContent className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 max-w-xs sm:max-w-md p-2 sm:p-4">
//           <DialogHeader>
//             <DialogTitle className="text-lg sm:text-xl">Reschedule Appointment</DialogTitle>
//           </DialogHeader>
//           <div className="space-y-3">
//             <div>
//               <Label className="block text-xs sm:text-sm font-medium text-muted-foreground mb-1">Appointment ID</Label>
//               <Input value={rescheduleAppointmentId || ''} readOnly className="text-xs sm:text-sm" />
//             </div>
//             <div>
//               <Label className="block text-xs sm:text-sm font-medium text-muted-foreground mb-1">New Date</Label>
//               <Input
//                 type="date"
...
//             </div>
//             <div>
//               <Label className="block text-xs sm:text-sm font-medium text-muted-foreground mb-1">New Time Slot</Label>
//               <Input
//                 type="time"
